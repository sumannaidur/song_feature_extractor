...
def download_audio(youtube_url, filename):
    ...
    output_path = f"audio_files/{filename}.wav"
    ydl_opts = {
...
def process_song(song):
    debug(f"Processing: {song['Title']} - {song['Artist']}")
        ...
        for _, row in df.iterrows():
            ...
            with ThreadPoolExecutor(max_workers=4) as executor:
                ...
                for future in as_completed(futures):
                    ...
                    if result:
                        folder = f"songs_by_year/{year}/{language}"
                        os.makedirs(folder, exist_ok=True)
                        target = os.path.join(folder, "features.csv")
...
@app.route("/")
def home():
    return "✅ Music pipeline is active! Use /download/{year}/{language} or /download_all"
...
if __name__ == "__main__":
    debug("🎬 Starting music pipeline...")
    process_all_movies()
    debug("✅ All songs processed. Launching Flask server...")
